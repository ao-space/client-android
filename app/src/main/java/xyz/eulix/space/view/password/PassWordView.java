...
package xyz.eulix.space.view.password;
...
public class PassWordView extends View {
    ...
    private void drawInputTextOrPicture(Canvas canvas) {
        if (isDrawText) {            //是否需要进行绘制
            ...
            switch (mShowPassType) {
                case 0:                                     //绘制圆心
                    mPaint.setColor(getResources().getColor(R.color.black_ff333333));
                    canvas.drawCircle((float) getMeasuredWidth() / 2, (float) getMeasuredHeight() / 2, 8, mPaint);
                    break;
                case 1:                                      //绘制*
                    mPaint.setTextSize((float) getMeasuredWidth() / 2 + 10);
                    float stringWidth = mPaint.measureText("*");

                    float baseY = ((float) getMeasuredHeight() / 2 - ((mPaint.descent() + mPaint.ascent()) / 2)) + stringWidth / 3;  //实现y轴居中方法
                    float baseX = (float) getMeasuredWidth() / 2 - stringWidth / 2;  //实现X轴居中方法
                ...
                case 2:                                     //绘制输入数据
                    ...
                    float baseY2 = ((float) getMeasuredHeight() / 2 - ((mPaint.descent() + mPaint.ascent()) / 2)) + stringWidth2 / 5;  //实现y轴居中方法
                    float baseX2 = (float) getMeasuredWidth() / 2 - stringWidth2 / 2;  //实现X轴居中方法
    ...
    private void drawRemindLine(Canvas canvas) {
        if (mDrawRemindLineState && isShowRemindLine) {  // mDrawRemindLineState 控制闪烁情况  //isShowRemindLine 是否绘制提示线
            ...
            canvas.drawLine((float)getMeasuredWidth() / 2, (float)getMeasuredHeight() / 2 - line_height / 2, getMeasuredWidth() / 2, getMeasuredHeight() / 2 + line_height / 2, mPaint);
        }
    }
    ...
    private void drawInputBox(Canvas canvas) {
        ...
        switch (mBoxDrawType) {
            case 1:         //绘制圆形
                canvas.drawCircle((float)getMeasuredWidth() / 2, (float)getMeasuredHeight() / 2, getMeasuredWidth() / 2 - 5, mPaint);
                break;
            case 2:        //绘制横线
                canvas.drawLine(0, (float)getMeasuredHeight(), (float)getMeasuredWidth(), (float)getMeasuredHeight(), mPaint);
                break;
            default:// 绘制矩形
                mPaint.setStyle(Paint.Style.FILL);//空心
                RectF rect = new RectF(0, 0, getMeasuredWidth(), (float)getMeasuredHeight());
                canvas.drawRoundRect(rect, getResources().getDimensionPixelSize(R.dimen.dp_4), getResources().getDimensionPixelSize(R.dimen.dp_4), mPaint);
        }
